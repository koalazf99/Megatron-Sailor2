...
def convert_wqkv_bias(llama_mega, n_hidden, layer_idx=0, n_heads=32, n_heads_kv=8):
    qkv_b = llama_mega["transformer"][f'layers.{layer_idx}.attention.query_key_value.bias']
    hidden_dim = n_hidden//n_heads
    qkv_b = permute_qkv_bias(qkv_b, n_hidden, n_heads, n_heads_kv, revert=True)

    n_qs_per_kv = n_heads//n_heads_kv
    n_groups = qkv_b.size(0)//hidden_dim//(n_qs_per_kv + 2)
    qkv_b = list(torch.split(qkv_b, hidden_dim, dim=0))
...
def write_qwen_model(
    ...
    with TemporaryDirectory() as tmp_model_path:
        ...
        state_dict = {
            "model.norm.weight": loaded["transformer"]['final_layernorm.weight'],
            "lm_head.weight": loaded['lm_head'] if size!=5 else loaded['embedding']["word_embeddings.weight"],
            # "lm_head.weight": loaded['lm_head'],
            "model.embed_tokens.weight": loaded['embedding']["word_embeddings.weight"]
        }
        ...
        # load qwen config from huggingface
        if size==14:
            config = AutoConfig.from_pretrained('Qwen/Qwen1.5-14B')
            # config = AutoConfig.from_pretrained('Qwen/Qwen1.5-14B-Chat')
            if 'rope1e6' in model_path:
                config.rope_theta = 1e6
            else:
                config.rope_theta = 1e4
            if '_sft_' in model_path:
            ...
            config = AutoConfig.from_pretrained('Qwen/Qwen1.5-4B')
        elif size==18:
            config = AutoConfig.from_pretrained('Qwen/Qwen1.5-1.8B')
        elif size==5:
        ...
        if size!=20 or size!=24 or size!=8:
            assert config.num_hidden_layers == n_layers
        ...
        print("Loading the checkpoint in a Qwen model...")
...
def write_tokenizer(args: Namespace):
    if args.model in {"llama", "llama2", "codellama", "mistral"}:
        ...
        if args.vocab_file:
            ...
            try:  # try loading from huggingface
                ...
                return
    elif args.model in {"falcon"}:
        hf_tokenizer = AutoTokenizer.from_pretrained("tiiuae/falcon-40b", cache_dir=args.cache_dir)
        args.tokenizer_type = "FalconTokenizer"
    elif args.model in {"qwen"}:
        if '_sft_' not in args.output_dir:
            hf_tokenizer = AutoTokenizer.from_pretrained('Qwen/Qwen2.5-0.5B', cache_dir=args.cache_dir)
            args.tokenizer_type = "Qwen2Tokenizer"
        else:
            hf_tokenizer = AutoTokenizer.from_pretrained('Qwen/Qwen2.5-0.5B-Instruct', cache_dir=args.cache_dir)
            args.tokenizer_type = "Qwen2ChatTokenizer"
    ...
    if args.tokenizer_type == "SentencePieceTokenizer":
        ...
        for t in tokens_to_check:
            ...
            b = hf_vocab.get(t)
            assert a == b, f"Mismatch between megatron and huggingface tokenizer vocabularies {t}, {a}, {b}"
        hf_tokenizer = AutoTokenizer.from_pretrained('Qwen/Qwen2.5-0.5B', split_special_tokens=False, clean_up_tokenization_spaces=False)
    elif args.tokenizer_type == "Qwen2ChatTokenizer":
        hf_tokenizer = mt_tokenizer.tokenizer
        additional_special_tokens = hf_tokenizer.additional_special_tokens
        special_tokens = {"additional_special_tokens": additional_special_tokens}
        if args.vocab_extra_ids_list:
        ...
        for t in tokens_to_check:
            ...
            b = hf_vocab.get(t)
            assert a == b, f"Mismatch between megatron and huggingface tokenizer vocabularies {t}, {a}, {b}"
        hf_tokenizer = AutoTokenizer.from_pretrained('Qwen/Qwen2.5-0.5B-Instruct', split_special_tokens=False, clean_up_tokenization_spaces=False)
...
def main():
    ...
    parser.add_argument("--model", choices={"falcon", "llama", "llama2", "codellama", "mistral", "qwen"},
                         default="llama2")
