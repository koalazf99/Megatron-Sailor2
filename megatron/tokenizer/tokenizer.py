...
def build_tokenizer(args):
    ...
    if args.tokenizer_type == 'BertWordPieceLowerCase':
        ...
        tokenizer = _HFTokenizer(args.vocab_file, vocab_extra_ids_list=args.vocab_extra_ids_list, new_tokens=args.new_tokens)
    elif args.tokenizer_type == 'Qwen2Tokenizer':
        tokenizer = _Qwen2Tokenizer(args.vocab_file, vocab_extra_ids_list=args.vocab_extra_ids_list, new_tokens=args.new_tokens)
    elif args.tokenizer_type == 'Qwen2ChatTokenizer':
        tokenizer = _Qwen2ChatTokenizer(args.vocab_file, vocab_extra_ids_list=args.vocab_extra_ids_list, new_tokens=args.new_tokens)
    else:
...
class _Qwen2Tokenizer(AbstractTokenizer):
    """Wrapper of huggingface tokenizer."""

    def __init__(self, vocab_file, vocab_extra_ids_list=None, new_tokens=True):
        name = 'Qwen2Tokenizer'
        super().__init__(name)
        self.tokenizer = AutoTokenizer.from_pretrained(vocab_file, split_special_tokens=False, clean_up_tokenization_spaces=False)
        self._eod_id = self.tokenizer.vocab['<|endoftext|>']
        self._pad_id = self.tokenizer.vocab['<|endoftext|>']

        if vocab_extra_ids_list and new_tokens:
            self.tokenizer.add_special_tokens({'additional_special_tokens': self.tokenizer.additional_special_tokens + vocab_extra_ids_list.split(",")})

        # self.tokenizer.add_special_tokens({'additional_special_tokens': self.tokenizer.additional_special_tokens + ['<|endoftext|>']})
        self._inv_vocab = {idx: token for token, idx in self.tokenizer.vocab.items()}
...
class _Qwen2ChatTokenizer(AbstractTokenizer):
    """Wrapper of huggingface tokenizer."""

    def __init__(self, vocab_file, vocab_extra_ids_list=None, new_tokens=True):
        name = 'Qwen2ChatTokenizer'
        super().__init__(name)
        self.tokenizer = AutoTokenizer.from_pretrained(vocab_file, split_special_tokens=False, clean_up_tokenization_spaces=False)
        self._eod_id = self.tokenizer.vocab['<|im_end|>']
        self._pad_id = self.tokenizer.vocab['<|endoftext|>']
...
class _HFTokenizer(AbstractTokenizer):
    ...
    def __init__(self, vocab_file, vocab_extra_ids_list=None, new_tokens=True):
        ...
        self._eod = self.tokenizer.vocab['</s>']
