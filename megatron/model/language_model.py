...
class TransformerLanguageModel(MegatronModule):
    ...
    def forward(self, 
        ...
        if self.intra_doc:
            # print(enc_input_ids)
            eod_ids = 151643
            ids = enc_input_ids.clone()
            ids[:, -1] = eod_ids
            ids = ids.view(-1)
            eod_index = torch.where(ids.eq(eod_ids))[0]
            assert eod_index.size(0) > 0, "eod must be input"
            eod_index = torch.cat([eod_index.new_zeros(1,), eod_index + 1], dim=0)
            seqlen = torch.diff(eod_index)
            max_len = seqlen.max().item()
            min_len = seqlen.min().item() 
            enc_attn_mask = eod_index.int(), max_len, min_len
